...
function EditGig() {
  ...
  const editGig = async () => {
    ...
    if (category && description && title && features.length && files.length && price > 0 && shortDesc.length && revisions > 0 && time > 0) {
      ...
      try {
        ...
        console.error("Error editing gig [gigId.jsx]:", error);
      }
    }
  };
  ...
  useEffect(()=>{
    const fetchGigData = async()=>{
        try{
            ...
            console.log("error fetching the GigData", err);
        }
    };
  ...
  return (
    <div className="min-h-[80vh] my-10 mt-0 px-32 ">
      ...
      <form action="" className="flex flex-col gap-5 mt-10">
        ...
        <div>
          <button onClick={editGig} type="button" className="border text-lg font-semibold px-5 py-3 border-[#1DBF73] bg-[#1DBF73] text-white rounded-md hover:bg-[#129357]">
            Edit Gig
          </button>
        </div>
      </form>
