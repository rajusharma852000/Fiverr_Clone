...
export const getGigData = async (req, res, next) => {
    try {
        if (req?.params?.gigId) {
            ...
            res.status(200).json({ gig: { ...gig, totalReviews: totalReviews, averageRating: averageRating } });
        }
        else {
            res.status(400).send("GigId is required")
        }
    }
    catch (err) {
...
export const deleteGig = async (req, res, next) => {
    try {
        const prisma = new PrismaClient();
        const deletedGig = await prisma.gigs.delete({
            where: { id: parseInt(req.params.gigId) },
        });
        if(!deletedGig){
            return res.status(400).send("Gig does not exist");
        }
...
export const searchGigs = async (req, res, next) => {
    try {
        if (req.query.searchTerm || req.query.category) {
            ...
            res.status(400).send("Search Term or Category is required")
        }
    }
    catch (err) {
        return res.status(500).send("Internal Sever Error.");
...
export const addReview = async (req, res, next) => {
    try {
        if (req.userId && req.params.gigId) {
            ...
            return res.status(400).send("You need to purchase the gig in order to add review.");
        }
        return res.status(400).send("userId and gigId is required.");
    } catch (err) {
        console.log(err);
